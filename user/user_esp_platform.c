 ...
 *      Author: asemenkov, Espressif Systems (Shanghai) Pte., Ltd.
 */
...
/******************************************************************************
 * FunctionName : user_esp_platform_parse_json_int_value
 * Description  : parse integer value by JSON key like "\"key: \""
 * Parameters   : bufferPtr -- the received data pointer
 * 				: jsonKeyPtr -- JSON key pointer
 * Returns      : parsed value
 *****************************************************************************/
int ICACHE_FLASH_ATTR user_esp_platform_parse_json_int_value(char *bufferPtr, char *jsonKeyPtr) {
	int value = 0;
	char *strPtr = NULL;

	strPtr = (char *) os_strstr(bufferPtr, jsonKeyPtr);

	if (strPtr != NULL) {
		strPtr += os_strlen(jsonKeyPtr);
		if (isdigit(*strPtr)) {
			value = atoi(strPtr);
		}
	}
...
//	uint32 stepsNum = 0;
	uint32 nonce = user_esp_platform_parse_json_int_value(bufferPtr, "\"nonce\": ");

	// Parse number of steps
	uint32 stepsNum = user_esp_platform_parse_json_int_value(bufferPtr, "\"steps_num\": ");
...
// Parse edge of pwm cycle decay
	uint32 pwmCycleDecayStop = user_esp_platform_parse_json_int_value(bufferPtr, "\"pwm_cycle_decay_stop\": ");
...
//	}
	...
	if (bufPtr != NULL) {
		if (stepsNum == 0) {
			...
			os_sprintf(bufPtr, ERROR_VALUE_FRAME, 423, "\"locked, whizzer is busy - steps left: ",
					user_get_remaining_macro_steps(), nonce);
			...
			os_sprintf(bufPtr, ERROR_VALUE_FRAME, 422, "\"PWM cycle decay threshold exceeded: greater than ",
			MAX_PWM_CYCLE_DECAY_STOP, nonce);
		} else if (pwmCycleDecayStop < MIN_PWM_CYCLE_DECAY_STOP) {
			os_sprintf(bufPtr, ERROR_VALUE_FRAME, 422, "\"short of PWM cycle decay value: less than ",
			MIN_PWM_CYCLE_DECAY_STOP, nonce);
		} else {
 ...
 * Returns      : none
 ******************************************************************************/
LOCAL void ICACHE_FLASH_ATTR user_esp_platform_upgrade_begin(struct espconn *espconnPtr,
		struct upgrade_server_info *serverPtr) {
	...
	os_sprintf(serverPtr->url,
			"GET /v1/device/rom/?action=download_rom&version=%s&filename=%s HTTP/1.0\r\nHost: "IPSTR":%d\r\n"pheadbuffer"",
			serverPtr->upgrade_version, userBin, IP2STR(serverPtr->ip), serverPtr->port, DEV_KEY);
	ESP_DBG("%s\n", serverPtr->url);
	...
	if (length == 1460) {
		...
		if ((strPtr = (char *) os_strstr(bufferPtr, "\"action\": \"sys_upgrade\"")) != NULL) {
			if ((strPtr = (char *) os_strstr(bufferPtr, "\"version\":")) != NULL) {
				...
				int nonce = user_esp_platform_parse_json_int_value(bufferPtr, "\"nonce\": ");
				...
				os_sprintf(serverPtr->pre_version, "%s%d.%d.%dt%d(%s)",
...
void ICACHE_FLASH_ATTR user_esp_platform_init(void) {
	...
	os_sprintf(iot_version, "%s%d.%d.%dt%d(%s)", VERSION_TYPE,
	IOT_VERSION_MAJOR,
	...
	if (resetInfo->reason == REASON_WDT_RST || resetInfo->reason == REASON_EXCEPTION_RST
		...
		os_printf("epc1=0x%08x, epc2=0x%08x, epc3=0x%08x, excvaddr=0x%08x, depc=0x%08x\n", resetInfo->epc1,
				resetInfo->epc2, resetInfo->epc3, resetInfo->excvaddr, resetInfo->depc);
	}
